...
- The picker is searchable and lists every zone with its current UTC offset, so you can type a city or an offset such as `+02`. Choosing **UTC** on purpose is the way to read the same clock as your server logs when correlating an event against them
