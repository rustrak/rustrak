...
| `digest.rate_limited` | Accepted events the quota dropped instead of storing |
...
| `quota_customized` | Whether any `MAX_EVENTS_*` limit differs from its default. The limits themselves are not sent |
